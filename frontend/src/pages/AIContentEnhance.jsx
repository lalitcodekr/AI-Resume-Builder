...
const AIEnhancementPage = () => {
  ...
  initial="hidden"
  animate="show"
 className="grid items-center min-h-[80vh] gap-4 pb-16 lg:grid-cols-[1fr_1.2fr] xl:grid-cols-[1fr_1.4fr]">
...
<motion.div variants={fadeUp} className="flex justify-center lg:justify-end">
  ...
  viewport={{ once: true }} className="max-w-4xl mx-auto">
          <motion.h2 variants={fadeUp} className="text-4xl font-bold text-center text-[#1a2e52] mb-12">What is AI Enhancement?</motion.h2>
  ...
  variants={fadeUp}
  whileHover={{ y: -6 }}
  transition={{ type: "spring", stiffness: 120 }} className="bg-[#f0f7ff] border-l-4 border-[#0077cc] rounded-2xl p-8 shadow-sm">
  ...
  initial="hidden"
  whileInView="show"
  viewport={{ once: true }} className="grid grid-cols-1 gap-8 md:grid-cols-3">
            {[
  ...
  variants={fadeUp}
  whileHover={{ y: -10 }}
  transition={{ type: "spring", stiffness: 200 }} className="p-10 rounded-[2.5rem] border border-gray-100 bg-white transition-all duration-300 hover:shadow-xl hover:-translate-y-1 group">
